import {
	...
	SOLFEGE_MAP,
	type Octave15
} from '$lib/translators/keybinds/15keys-double';
...
function parseNote(token: string): ParsedNote | null {
	...
	let octave: Octave15 = 'middle';
	let noteStr = strippedToken;

	// 15-key layout has no low octave, so '.' is snapped to middle.
	...
	if (!note) {
		...
		if (accidental === 'b') {
			note = FLAT_TO_15_KEY[`${note}b`] ?? note;
		} else if (accidental === '#') {
			note = SHARP_TO_15_KEY[`${note}#`] ?? note;
		}
	}

	if (!KEYS_15_KEYBINDS[octave][note]) {
		if (octave === 'highest') {
			octave = 'top';
		}
		if (!KEYS_15_KEYBINDS[octave][note]) {
...
function getKeybind(parsedNote: ParsedNote | null): string | null {
	...
	return KEYS_15_KEYBINDS[octave][note] ?? null;
}
...
export function translate15KeysDoubleNotes(input: string, options: TranslateOptions = {}): string {
...
export const keys15DoubleTranslator: InstrumentTranslator = {
	exportTitle: 'Heartopia 15 Keys (Double Row) Keybinds',
	translate: (input, options = {}) => translate15KeysDoubleNotes(input, options)
};
